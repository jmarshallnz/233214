...
Once we have the weights $w_j$, we then compute the interpolated measure $\hat{Z}$ using the same equation as was used for [IDW](week12_idw.html):
$$
\hat{Z} = \frac{\sum_{j=1}^n w_jz_j}{\sum_{j=1}^n w_j}
$$
