...
Complete spatial randomness definitely doesn't apply, as the points are clustered. So a homogeneous Poisson process would not be appropriate - we'd need an inhomogeneous process that varies through space.
...
Yes, the larynx caess do seem to follow roughly the same distribution as lung cases, possibly as both are mostly driven by where people are.
...
There does some to be some areas where the density of larynx cases is higher than the density of lung cases, e.g. in the south.
...
The adaptive estimate seems to be better - particularly for the larynx data. Regions where there aren't many observations are undersmoothed with the fixed bandwidth, and this is remedied in the adaptive estimate.
...
The reason that we use a log scale for risk is that we want a double risk of larynx compared to lung to be represented by a similar magnitude as the double risk of lung compared to larynx. On a log scale these magnitudes are the same (i.e. log(2) is the same distance from log(1) as log(1/2) is).
...
Maybe. There are some areas of increased risk in the south and in the west. But, we've only got a few cases there (2 in the west and maybe 4 in the south?). So we should be a little cautious of claiming increased risk here without other knowledge being available to us.
...
The p-value tolerance contours are saying that there is some statistical evidence for these two regions (i.e. it's unlikely to be pure chance) but we should still be a little cautious particularly in the spot where we have only two cases.
...
The data itself was telling us to be suspicious of the cluster in the south - there did seem to be some evidence of increased larynx cancer risk there. That the incinerator is also there might give a plausible link or rationale for the increased larynx cancer rates - definitely something to look into more.
...
The reason is that the ppm model linear predictor is for log(lambda), i.e. the log intensity. So we want our density (or intensity) surface for lung cancer to also be on the log scale.
...
We don't have statistical evidence (a significant P-value) for the effect of the incinerator after adjusting for the intensity of lung cancer.
...
Overall, probably yes. It seems plausible that the incinerator could be responsible at least in part for the cases in the south, as we do have a tightly clusted set of cases right near the incinerator. However, it's still only 3-4 cases, so we may not have strong statistical evidence for this. Certainly enough evidence to look a little closer though!
