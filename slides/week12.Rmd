...
The power $p$ controls how quickly the influence of points further away drops off.
 - Smaller values $p < 2$ allow distance points to have more influence.
 - Larger values $p > 2$ mean distance points have little or no influence.
...
We then supply the variable we want and the new data locations we want to interpolate to, along with the power to use:
...
and plots this against the distance $d_{ij}$ between observations $i$ and $j$, for all pairs of observations.
...
Once we have plotted our variogram, we need to fit a variogram model $\gamma(d)$. There are many options:
...
In some situations we might know that the underlying spatial trend correlates with some other measure, which we have available to us across the region where we wish to interpolate.
...
Assume we have a covariate that can be used to predict the spatial trend. We implement universal kriging as follows:
...
2. Use that statistical model to predict the average spatial trend across ther egion.
...
The log zinc concentrations were closest near the river.
...
.right-plot[
```{r, ref.label="dist3", message=FALSE}
```
]
...
- Universal kriging adds a parametric model for the trend, so is useful if you know the measure that is only available at sampled locations is correlated with things you have measured everywhere.
